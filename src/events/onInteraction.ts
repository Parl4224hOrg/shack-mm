    ...
    try {
        if (interaction.isChatInputCommand()) {
            ...
            if (permission.limited) {
                ...
                await interaction.reply({flags: MessageFlagsBitField.Flags.Ephemeral, content: `Please use this modal in a valid channel\nValid channels: ${getChannels(modal.allowedChannels!)}`});
            } else if (permission.guild) {
                await interaction.reply({flags: MessageFlagsBitField.Flags.Ephemeral, content: "This modal cannot be used in this server"});
            } else if (permission.valid) {
                await modal.run(interaction, data);
            } else {
                await interaction.reply({flags: MessageFlagsBitField.Flags.Ephemeral, content: "You do not have permission to use this modal"});
            }
        }
    } catch (e) {
